...
// ============================================
// ============================================
// DESENHAR ACORDE DE PIANO PERSONALIZADO (CORRIGIDO)
// ============================================
function desenharAcordePianoPersonalizado(container, sigla, nome) {
    ...
    // CORRIGIDO: usa 'notas' que já tem a oitava
    const notasAtivas = acorde.notas || [];
    const dedos = acorde.fingersTreble ? acorde.fingersTreble.split(/\s+/) : [];
    const startOitava = acorde.startOitava || 'C3';
    const endOitava = acorde.endOitava || 'C5';
    const nomeExibido = acorde.nome || nome;
    
    // DEBUG: Ver o que está sendo passado
    console.log("🎹 Desenhando PIANO-CUSTOM:", { 
        sigla, 
    ...
    // Chama a função de desenho que está no main.js
    if (typeof desenharTecladoPiano === 'function') {
        desenharTecladoPiano(container, sigla, nomeExibido, notasAtivas, startOitava, endOitava, dedos);
    } else {
...
function renderizar() {
    ...
    try {
        ...
        // Dentro da função renderizar(), substitua a parte dos pianos por:

pianos.forEach(p => {
    const el = document.getElementById(p.id);
    if (el) {
        // Prioriza a função do acordes_piano.js
        if (typeof desenharAcordePianoPreview === 'function') {
            desenharAcordePianoPreview(el, p.sigla, p.nome);
        } 
        // Fallback para o método antigo
        else if (typeof desenharTecladoPiano === 'function' && ACORDES_PIANO && ACORDES_PIANO[p.sigla]) {
            const a = ACORDES_PIANO[p.sigla];
            desenharTecladoPiano(el, p.sigla, a.nome, a.notas, a.startOitava, a.endOitava, a.dedosTreble);
        }
        else {
            el.innerHTML = `<div style="color:red; padding:10px;">❌ Não foi possível desenhar o acorde de piano "${p.sigla}"</div>`;
        }
    }
});
...
function inserirPiano() {
    // Listar siglas disponíveis
    const siglasDisponiveis = Object.keys(ACORDES_PIANO).join(', ');
    const sigla = prompt(`Sigla do acorde de piano:\n\nDisponíveis: ${siglasDisponiveis}\n\nDigite a sigla:`, 'C');
    if (!sigla) return;
...
// ============================================
// SISTEMA DE BACKUP - 3 OPÇÕES
// ============================================

// 1️⃣ ARQUIVO SIMPLES
function exportarArquivo() {
    if (listaAtual === null) {
        toast('❌ Abra uma pasta primeiro (clique em um cartão)!', 'error');
        return;
    }
    
    const pastaAtual = getListaByPath(listaAtual);
    if (!pastaAtual) {
    ...
    const dadosPasta = {
        ...
        nomeLista: pastaAtual.nome,
        cards: pastaAtual.cards || [],
        totalCards: pastaAtual.cards ? pastaAtual.cards.length : 0
    };
    
    const jsonStr = JSON.stringify(dadosPasta, null, 2);
    const blob = new Blob([jsonStr], { type: 'application/json' });
    const url = URL.createObjectURL(blob);
    
    // PEDIR NOME DO ARQUIVO
    const nomePadrao = `pasta_${pastaAtual.nome.replace(/[^a-z0-9]/gi, '_')}`;
    const nomePersonalizado = prompt('Nome do arquivo (sem extensão):', nomePadrao);
    const nomeFinal = nomePersonalizado && nomePersonalizado.trim() !== '' 
        ? nomePersonalizado.replace(/[^a-z0-9_-]/gi, '_') 
        : nomePadrao;
    
    const a = document.createElement('a');
    a.href = url;
    a.download = `${nomeFinal}.json`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
    
    toast(`✅ ${dadosPasta.totalCards} cartões exportados como "${nomeFinal}.json"!`, 'success');
}

function importarArquivo() {
    if (listaAtual === null) {
        toast('❌ Abra uma pasta primeiro!', 'error');
        return;
    }
    ...
    input.onchange = function(event) {
        ...
        reader.onload = function(e) {
            try {
                ...
                const pastaDestino = getListaByPath(listaAtual);
                ...
                if (dadosImportados.cards) {
                    const opcao = confirm(`Importar ${dadosImportados.cards.length} cartões?\nOK=Adicionar | Cancelar=Substituir`);
                    
                    if (opcao) {
                        if (!pastaDestino.cards) pastaDestino.cards = [];
                        pastaDestino.cards.push(...dadosImportados.cards);
                        toast(`✅ ${dadosImportados.cards.length} cartões adicionados!`, 'success');
                    } else {
                        pastaDestino.cards = dadosImportados.cards;
                        toast(`✅ ${dadosImportados.cards.length} cartões substituídos!`, 'success');
                    }
...
// 2️⃣ PASTA ATUAL (COM RENOMEAR)
function exportarPastaAtual() {
    if (listaAtual === null) {
        toast('❌ Abra uma pasta primeiro!', 'error');
        return;
    }
    ...
    const dadosPasta = {
        ...
        nomeLista: pastaAtual.nome,
        cards: pastaAtual.cards || [],
        subListas: pastaAtual.subListas || [],
        totalCards: pastaAtual.cards ? pastaAtual.cards.length : 0,
        totalSubPastas: pastaAtual.subListas ? pastaAtual.subListas.length : 0
    };
    ...
    // PEDIR NOME DO ARQUIVO
    const nomePadrao = `pasta_completa_${pastaAtual.nome.replace(/[^a-z0-9]/gi, '_')}`;
    const nomePersonalizado = prompt('Nome do arquivo (sem extensão):', nomePadrao);
    const nomeFinal = nomePersonalizado && nomePersonalizado.trim() !== '' 
        ? nomePersonalizado.replace(/[^a-z0-9_-]/gi, '_') 
        : nomePadrao;
    ...
    toast(`✅ Pasta "${pastaAtual.nome}" exportada como "${nomeFinal}.json"!`, 'success');
}

function importarParaPastaAtual() {
    if (listaAtual === null) {
        toast('❌ Abra uma pasta primeiro!', 'error');
        return;
    }
    ...
    input.onchange = function(event) {
        ...
        reader.onload = function(e) {
            try {
                ...
                if (dadosImportados.tipo !== 'pasta_atual') {
                    toast('❌ Arquivo inválido!', 'error');
                    return;
                }
                
                const opcao = confirm(`Importar para "${pastaDestino.nome}"?\nOK=Substituir | Cancelar=Adicionar`);
                
                if (opcao) {
                    pastaDestino.cards = dadosImportados.cards || [];
                    pastaDestino.subListas = dadosImportados.subListas || [];
                } else {
                    if (dadosImportados.cards) {
                        if (!pastaDestino.cards) pastaDestino.cards = [];
                        pastaDestino.cards.push(...dadosImportados.cards);
                    }
                    if (dadosImportados.subListas) {
                        if (!pastaDestino.subListas) pastaDestino.subListas = [];
                        pastaDestino.subListas.push(...dadosImportados.subListas);
                    }
                }
...
// 3️⃣ SISTEMA COMPLETO (COM RENOMEAR)
function exportarTudoSistema() {
    const dadosCompletos = {
        tipo: 'completo_sistema',
        versao: '1.0',
        dataExportacao: new Date().toISOString(),
        dados: dados,
        bibliotecaAcordes: typeof bibliotecaAcordes !== 'undefined' ? bibliotecaAcordes : {}
    };
    ...
    // PEDIR NOME DO ARQUIVO
    const nomePadrao = `pro_maestro_completo_${new Date().toISOString().slice(0,19).replace(/:/g, '-')}`;
    const nomePersonalizado = prompt('Nome do arquivo (sem extensão):', nomePadrao);
    const nomeFinal = nomePersonalizado && nomePersonalizado.trim() !== '' 
        ? nomePersonalizado.replace(/[^a-z0-9_-]/gi, '_') 
        : nomePadrao;
    ...
    toast(`✅ Backup completo exportado como "${nomeFinal}.json"!`, 'success');
}

function importarTudoSistema() {
    const input = document.createElement('input');
    input.type = 'file';
    input.accept = '.json';
    input.onchange = function(event) {
        ...
        reader.onload = function(e) {
            try {
                ...
                if (dadosImportados.tipo !== 'completo_sistema') {
                    toast('❌ Arquivo inválido!', 'error');
                    return;
                }
                
                if (confirm('ATENÇÃO! Isso irá substituir TODOS os dados. Continuar?')) {
                    if (dadosImportados.dados) {
                        dados = dadosImportados.dados;
                        localStorage.setItem(STORAGE_KEY, JSON.stringify(dados));
                    }
                    if (dadosImportados.bibliotecaAcordes && typeof bibliotecaAcordes !== 'undefined') {
                        bibliotecaAcordes = dadosImportados.bibliotecaAcordes;
                        localStorage.setItem('biblioteca_acordes', JSON.stringify(bibliotecaAcordes));
                        if (typeof atualizarBibliotecaVisual === 'function') atualizarBibliotecaVisual();
                    }
                    renderizarListaAulas();
                    if (typeof renderizar === 'function') renderizar();
                    toast('✅ Sistema restaurado!', 'success');
...
function adicionarBotoesBackup() {
    ...
    backupDiv.style.background = '#0f3460';
    backupDiv.style.borderRadius = '8px';
    
    backupDiv.innerHTML = `
        <h4 style="color: #e94560; margin-bottom: 10px;">💾 Backup</h4>
        <div style="display: flex; flex-direction: column; gap: 8px;">
            <div style="background: #1a1a2e; padding: 8px; border-radius: 5px;">
                <div style="font-size: 12px; color: #aaa;">📄 Arquivo Simples</div>
                <div style="display: flex; gap: 8px;">
                    <button onclick="exportarArquivo()" style="flex:1; background:#2ecc71; padding:8px; border:none; border-radius:5px; color:white; cursor:pointer;">📤 Exportar</button>
                    <button onclick="importarArquivo()" style="flex:1; background:#3a86ff; padding:8px; border:none; border-radius:5px; color:white; cursor:pointer;">📥 Importar</button>
                </div>
            </div>
            <div style="background: #1a1a2e; padding: 8px; border-radius: 5px;">
                <div style="font-size: 12px; color: #aaa;">📂 Pasta Atual</div>
                <div style="display: flex; gap: 8px;">
                    <button onclick="exportarPastaAtual()" style="flex:1; background:#9b59b6; padding:8px; border:none; border-radius:5px; color:white; cursor:pointer;">📤 Exportar</button>
                    <button onclick="importarParaPastaAtual()" style="flex:1; background:#e67e22; padding:8px; border:none; border-radius:5px; color:white; cursor:pointer;">📥 Importar</button>
                </div>
            </div>
            <div style="background: #1a1a2e; padding: 8px; border-radius: 5px;">
                <div style="font-size: 12px; color: #aaa;">💾 Sistema Completo</div>
                <div style="display: flex; gap: 8px;">
                    <button onclick="exportarTudoSistema()" style="flex:1; background:#e94560; padding:8px; border:none; border-radius:5px; color:white; cursor:pointer;">📤 Exportar</button>
                    <button onclick="importarTudoSistema()" style="flex:1; background:#f39c12; padding:8px; border:none; border-radius:5px; color:white; cursor:pointer;">📥 Importar</button>
                </div>
            </div>
        </div>
        <p style="font-size: 11px; margin-top: 10px; color: #aaa; text-align: center;">
            📄 <strong>Arquivo</strong> = Apenas os cards da pasta atual<br>
            📂 <strong>Pasta Atual</strong> = Cards + subpastas<br>
            💾 <strong>Sistema</strong> = Todas as pastas
        </p>
    `;
    
    const sidebarContent = sidebar.querySelector('.sidebar-content');
    if (sidebarContent) {
        const controles = sidebarContent.querySelector('.controles-sidebar');
        if (controles) {
            sidebarContent.insertBefore(backupDiv, controles);
        } else {
            sidebarContent.appendChild(backupDiv);
        }
    }
}
...
function init() {
    ...
    carregarDados();
    adicionarBotoesBackup(); // <--- ADICIONE ESTA LINHA
