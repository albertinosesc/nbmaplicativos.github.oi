...
// ============================================================
// ELEMENTOS DOM (APENAS OS QUE EXISTEM)
// ============================================================
...
// ============================================================
// PDF - GOOGLE DRIVE (abre em nova aba)
// ============================================================
...
function onYouTubeIframeAPIReady() {
    ...
    // O player será criado sob demanda em playMusic
}
...
function playMusic(musicId, forceMp3 = false) {
    ...
    // ===== YOUTUBE =====
    if (ytId) {
        // FUNÇÃO PARA CRIAR IFRAME (fallback)
        function criarIframe(videoId) {
        ...
        if (playerLeft) playerLeft.appendChild(container);

        // Se a API já está disponível, tenta o player
        if (typeof YT !== 'undefined' && YT.Player) {
            // Tenta criar o player da API
            try {
                ...
                globalPlayer = new YT.Player('youtube-player-container', {
                    ...
                    events: {
                        ...
                        onError: function(e) {
                            ...
                            // Fallback: troca para iframe
                            if (container) {
                                container.innerHTML = '';
                                const iframe = criarIframe(ytId);
                ...
                console.error('Falha ao criar player API:', e);
        ...
        // Se a API não está disponível, aguarda ou usa iframe imediatamente
        container.innerHTML = '<div style="color:#fff;font-size:1rem;">Carregando player...</div>';
        
        // Aguarda a API por 3 segundos, se não carregar, usa iframe
        const timeoutId = setTimeout(() => {
            if (!globalPlayer) {
                console.warn('API do YouTube não carregou a tempo, usando iframe.');
                if (container) {
                    container.innerHTML = '';
                    const iframe = criarIframe(ytId);
                    container.appendChild(iframe);
        ...
        window.onYouTubeIframeAPIReady = function() {
            ...
            if (playerLeft) {
                ...
                newContainer.id = 'youtube-player-container';
                newContainer.style.width = '100%';
                newContainer.style.height = '100%';
                newContainer.style.display = 'flex';
                newContainer.style.alignItems = 'center';
                newContainer.style.justifyContent = 'center';
                newContainer.style.background = '#111';
                newContainer.style.color = '#fff';
                newContainer.style.flexDirection = 'column';
                newContainer.style.padding = '20px';
                newContainer.style.textAlign = 'center';
                playerLeft.appendChild(newContainer);
...
function renderCatalog() {
    console.log('renderCatalog chamado');
    if (typeof musicDatabase === 'undefined') {
        console.warn('musicDatabase não definido');
        if (catalogGrid) catalogGrid.innerHTML = '<p>Erro: banco de dados não carregado.</p>';
        return;
    }
    ...
    console.log('Catálogo renderizado com', filtered.length, 'itens');
}
...
if (btnClear) {
    btnClear.addEventListener('click', () => {
        ...
        if (viewModeRadios) viewModeRadios[0].checked = true;
        renderCatalog();
    });
}
