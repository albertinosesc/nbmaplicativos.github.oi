...
// ============================================
// ALTERNAR QUEBRAS DE LINHA NO PREVIEW
// ============================================
function toggleQuebraDeLinha() {
    const preview = document.getElementById('preview');
    const btn = document.getElementById('btnQuebras');
    
    if (preview.style.whiteSpace === 'pre-wrap') {
        preview.style.whiteSpace = 'normal';
        if (btn) {
            btn.style.background = '#3a86ff';
            btn.innerHTML = '¶ Quebras OFF';
        }
    } else {
        preview.style.whiteSpace = 'pre-wrap';
        if (btn) {
            btn.style.background = '#2ed573';
            btn.innerHTML = '¶ Quebras ON';
        }
    }
}

// Criar e adicionar o botão automaticamente
function criarBotaoQuebraLinha() {
    // Procura a toolbar
    let toolbar = document.querySelector('.editor-toolbar');
    
    // Se não existir toolbar, cria uma
    if (!toolbar) {
        toolbar = document.createElement('div');
        toolbar.className = 'editor-toolbar';
        toolbar.style.cssText = 'display: flex; gap: 5px; padding: 10px; background: #16213e; border-radius: 5px; margin-bottom: 10px; flex-wrap: wrap;';
    ...
    if (document.getElementById('btnQuebras')) return;
    
    // Cria o botão
    const btn = document.createElement('button');
    btn.id = 'btnQuebras';
    btn.innerHTML = '¶ Quebras OFF';
    btn.title = 'Alternar quebras de linha no preview';
    btn.onclick = toggleQuebraDeLinha;
    btn.style.cssText = `
        background: #3a86ff;
        color: white;
        border: none;
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', criarBotaoQuebraLinha);
} else {
    criarBotaoQuebraLinha();
}
